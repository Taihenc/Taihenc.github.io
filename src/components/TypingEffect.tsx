...
const TypingEffect: React.FC<TypingEffectProps> = ({
  ...
  onCharTyped,
  isPlainText,
  isPlainTextWithCursor,
  textHighlight,
  deleteAndReType,
  speed,
}) => {
  const [typedText, setTypedText] = useState<string[]>([])
  const codeRefs = Array.from({ length: text.length }, () =>
    ...
    const intervalId = setTimeout(
      async () => {
        if (currentTypedIndex < text.length) {
          if (!deleteAndReType)
            codeRefs[currentIndex - 1]?.current?.classList.remove(
              'cursor',
              'terminal'
            )
          ...
          if (currentTypedText.length < currentText.length) {
            ...
            if (textHighlight) {
              textHighlight.forEach((wordWithColor) => {
                ...
                if (codeRef) {
                  codeRef.innerHTML = codeRef.innerHTML.replace(
                    regex,
    ...
    <>
      {typedText
        ...
        .map((typed, index) => (
          <pre
            key={index}
            data-prefix={isTerminal ? `$` : `${index + 1}`}
            className="w-full relative inline-flex whitespace-pre-wrap break-all"
          >
            <code
              ref={codeRefs[index]}
              className={isTerminal ? 'terminal text-green-200' : 'cursor'}
            >
              {typed}
            </code>
          </pre>
