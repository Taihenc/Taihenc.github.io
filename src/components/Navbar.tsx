...
export default function NavBar() {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        entries.forEach((entry) => {
          const navItemIndex = updatedNavigation.findIndex(
            (navItem) => navItem.href === `#${entry.target.id}`
          )
  ...
  return (
    <Disclosure
      as="nav"
      id="navbar"
      className="bg-transpalent fixed z-10 w-full top-0 flex justify-end sm:justify-center md:pt-4"
    >
      {({ open }) => (
                    ...
                    {navigation.map((item) => (
                      <a
                        ...
                        className={classNames(
                          item.current
                            ? 'bg-[#223b93] text-white'
                            : 'text-gray-300 hover:text-white hover:bg-[#433969] bg-transparent',
